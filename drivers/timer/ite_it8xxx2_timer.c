...
BUILD_ASSERT(IS_ENABLED(CONFIG_TICKLESS_KERNEL),
		"ITE timer doesn't support non-tickless kernel");
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	...
	if (ticks == K_TICKS_FOREVER) {
		...
		/*
		 * Ticks <= 0 means the kernel wants the tick announced
...
int sys_clock_driver_init(const struct device *dev)
{
	...
	ret = timer_init(FREE_RUN_TIMER, EXT_PSR_32P768K, TRUE, FREE_RUN_TIMER_MAX_CNT,
			TRUE, FREE_RUN_TIMER_IRQ, FREE_RUN_TIMER_FLAG, FALSE, TRUE);
	if (ret < 0) {
	...
	ret = timer_init(EVENT_TIMER, EXT_PSR_32P768K, TRUE, EVENT_TIMER_MAX_CNT,
			TRUE, EVENT_TIMER_IRQ, EVENT_TIMER_FLAG, FALSE, FALSE);
	if (ret < 0) {
